...
public class MazeApplication {
    ...
    public static void main(String[] args) throws FileNotFoundException {
        ...
        while (!list.isEmpty()) {
            ...
            /*solution[current.i][current.j]= (char) */
            mark(current, path);
            //add its neighbors in the queue
            next = current.north();    //move up
            if (isInMaze(next) && isClear(next)&&((N==1||W==1)&&S==0)/*||Flag==0*/) { 
                list.add(next);
                //Flag = 1;
                N=1;
                E=0;
            ...
            if (isInMaze(next) && isClear(next)&&(E==1||N==1)&&W==0) {
                list.add(next);
                N=0;
                E=1;
                S=0;
            ...
            if (isInMaze(next) && isClear(next)&&(S==1||E==1)&&N==0) {
                list.add(next);
                E=0;
                W=0;
                S=1;
            ...
            if (isInMaze(next) && isClear(next)&&(W==1||S==1)&&E==0) {
                list.add(next);
                N=0;
                S=0;
                W=1;
        ...
        if (!list.isEmpty()) {               //you exited before the list was emptied, meaning that you found the solution and exited.
         ...
         for (int i = 0; i < sizerow ; i++) {      //go to every row
    ...
    //functions
    public static void start_End_points(char [][] matrix,int row_size,int col_size){
        for(int i=0 ; i<row_size ; i++){
            for(int j=0 ; j<col_size ; j++){
                if (matrix[i][j]=='c'){
                    ...
                    start_j = j;
                    solution[i][j]='c';
                    System.out.println("Start point coordinations ("+i +","+ j+")");
                }
                if(matrix[i][j]=='e'){
                    exit_i = i;
                    exit_j = j;
                    solution[i][j]='e';
                    System.out.println("Goal coordinations ("+i +","+ j+")");
                }
            }
        }
    }
    static public boolean isFinal (int i, int j){
        boolean x;
    ...
    static public boolean isClear(int i, int j) {
        assert (isInMaze(i, j));
